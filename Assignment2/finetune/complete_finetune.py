...
def load_and_prepare_data(model_name, max_length=None):
    ...
    # Use imdb['train'] for training and imdb['test'] for evaluation
    train_split = tokenized_imdb["train"]
...
def run_finetuning(model_name, method):
    ...
    max_length = 492 if method == "prefix" else None
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=5,  # Changed to 2 epochs
        weight_decay=0.01,
    ...
    if os.path.exists(dirs["FINAL_MODEL_PATH"]) and any(os.listdir(dirs["FINAL_MODEL_PATH"])):
        ...
        trainer = Trainer(
            ...
            eval_dataset=data["test_split"],  # Using test split for evaluation
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run finetuning on different models with various methods')
