...
MODEL_NAME = "bert-augment-synthetic"  # Change this to your desired model
...
def load_and_prepare_data():
    ...
    train = load_dataset("HamsterShiu/IMDB_Combined")
    ...
    # Tokenize dataset
    tokenized_test = test.map(lambda e: tokenizer(e["text"], truncation=True, padding=True), batched=True)
    tokenized_train = train.map(lambda e: tokenizer(e["text"], truncation=True, padding=True), batched=True)
    ...
    train_val_split = tokenized_train["train"].train_test_split(test_size=0.2)
    train_split = train_val_split["train"]
    val_split = train_val_split["test"]
    test_split = tokenized_test
